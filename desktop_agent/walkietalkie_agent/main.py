...
async def _async_main() -> None:
    ...
    target_kind = settings.surface_kind.lower()
    if target_kind == "cli":
        target = get_cli_target(settings.desktop_target)
        logger.info("agent.target", target=target.key, surface=target_kind, command=target.command)
    else:
        target = get_desktop_target(settings.desktop_target)
        logger.info("agent.target", target=target.key, surface=target_kind, app_name=target.app_name)
    ...
    injector = get_injector(settings.desktop_target, settings.surface_kind)
    response_watcher = (
        ResponseWatcher(server.send_tts, target.response_capture)
        if hasattr(target, "response_capture") and getattr(target, "response_capture") is not None
        else None
    )
